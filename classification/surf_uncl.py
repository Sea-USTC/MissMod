...
from models.surf_baseline import SURF_Multi, SURF_UNCLNet, SURF_UNCLLateNet
from loss.kd import *
from lib.model_develop import train_uncl_patch_feature
...
def deeppix_main(args):
    ...
    if args.dataset=='surf':
        teacher_model.load_state_dict(
            torch.load(os.path.join(args.model_root, 'multi_full__1__average.pth')))
    elif args.dataset=='cefa':
        teacher_model.load_state_dict(
            torch.load(os.path.join(args.model_root, 'cefa_full__2__unaverage.pth')))
    else:
        raise Exception('error dataset')
    ...
    criterions = {'criterionCls': criterionCls, 'criterionKD': criterionKD, 'criterionRe':criterionRe}

    train_uncl_patch_feature(net_dict=nets, cost_dict=criterions, optimizer=optimizer,
