...
class GaussianBlur(object):
    ...
    def __call__(self, x):
        image_x, image_ir, image_depth, binary_label = x['image_x'], x['image_ir'], x[
            'image_depth'], x['binary_label']
        sigma = random.uniform(self.sigma[0], self.sigma[1])
        if random.uniform(0, 1) < self.p:
            image_x = Image.fromarray(cv2.cvtColor(image_x,cv2.COLOR_BGR2RGB)) 
            new_image_x = image_x.filter(ImageFilter.GaussianBlur(radius=sigma))
            new_image_x = cv2.cvtColor(np.asarray(new_image_x),cv2.COLOR_RGB2BGR)
        else:
            new_image_x = image_x

        if random.uniform(0, 1) < self.p:
            image_ir = Image.fromarray(cv2.cvtColor(image_ir,cv2.COLOR_BGR2RGB)) 
            new_image_ir = image_ir.filter(ImageFilter.GaussianBlur(radius=sigma))
            new_image_ir = cv2.cvtColor(np.asarray(new_image_ir),cv2.COLOR_RGB2BGR)
        else:
            new_image_ir = image_ir

        if random.uniform(0, 1) < self.p:
            image_depth = Image.fromarray(cv2.cvtColor(image_depth,cv2.COLOR_BGR2RGB)) 
            new_image_depth = image_depth.filter(ImageFilter.GaussianBlur(radius=sigma))
            new_image_depth = cv2.cvtColor(np.asarray(new_image_depth),cv2.COLOR_RGB2BGR)
        else:
            new_image_depth = image_depth
