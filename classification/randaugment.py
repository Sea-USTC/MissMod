...
def randaugment_list():
    ...
    # augs = [
    #     (Rotate, 0, 30), (Posterize, 4, 8),(Sharpness, 0.1, 1.9), (ShearX, 0., 0.3),
...
class RandAugment_prob:
    ...
    def __call__(self, img):
        image_x, image_ir, image_depth, binary_label = img['image_x'], img['image_ir'], img[
            'image_depth'], img['binary_label']
        ...
        for op, minval, maxval in ops:
            if random.uniform(0, 1) < self.p:
                val = (float(self.m) / 30) * float(maxval - minval) + minval
                new_image_depth = op(image_depth, val)
            else:
