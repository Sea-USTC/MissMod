...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.layer1(x)
        x = self.layer2(x)
        selayer = self.se_layer(x)
        x = self.layer3(selayer)
        x = self.layer4(x)
...
def resnet18_se_trm(args, pretrained=False, **kwargs):
...
def resnet18_uni_trm(args, pretrained=False, **kwargs):
    """Constructs a ResNet-18 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet_uni(BasicBlock, [2, 2, 2, 2], args=args, num_classes=args.class_num, **kwargs)
    if not pretrained:
        return model
    else:
        resnet_18_pretrain = tm.resnet18(pretrained=True)
...
class ResNet_uni(nn.Module):

    def __init__(self, block, layers, args, num_classes=1000, zero_init_residual=False,
                 groups=1, width_per_group=64, replace_stride_with_dilation=None,
                 norm_layer=None):
        super(ResNet_uni, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        self._norm_layer = norm_layer
        self.args = args
